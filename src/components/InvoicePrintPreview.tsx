...
export const InvoicePrintPreview = ({ invoice, onBack }: InvoicePrintPreviewProps) => {
  ...
  const [pages, setPages] = useState<PreviewPage[]>([
    {
      items: invoice.items,
      tailBlocks: ['summary', ...(hasNotes ? ['notes' as TailBlockKey] : []), 'signature'],
    },
  ]);
    ...
    <div className="invoice-print-root min-h-screen bg-muted">
      <div className="preview-toolbar print:hidden p-4 bg-background border-b flex gap-4 items-center">
        ...
        <div className="ml-auto text-sm text-muted-foreground">
          Pré-visualização paginada em A4
        </div>
      </div>
        ...
        {pages.map((page, pageIndex) => (
          ...
          >
            <div
              ...
              style={{ minHeight: `${A4_HEIGHT_MM - PAGE_PADDING_MM * 2}mm` }}
            >
              <PreviewHeader
                invoice={invoice}
              ...
              <PageLabel pageNumber={pageIndex + 1} totalPages={pages.length} />
            </div>
          </div>
        ))}
      </div>
